...
class ModuleImplementsAlterTest extends KernelTestBase
{
   ...
   */
    ...
    // Get an instance of the module handler, to observe how it is going to be
        ...
        $this->assertTrue(
            function_exists('module_test_modules_installed'),
        ...
        $this->assertContains(
            'module_test',
            \Drupal::moduleHandler()->getImplementations('modules_installed'),
            'module_test implements hook_modules_installed().'
        );

        $this->assertContains(
            'module_test',
            \Drupal::moduleHandler()->getImplementations('module_implements_alter'),
            'module_test implements hook_module_implements_alter().'
        );

        // Assert that module_test.implementations.inc is not included yet.
        $this->assertFalse(
            function_exists('module_test_altered_test_hook'),
        ...
        $this->assertContains(
            'module_test',
            \Drupal::moduleHandler()->getImplementations('altered_test_hook'),
