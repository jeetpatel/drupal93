...
/**
 * Represents Create Employee records as resources.
 *
 * @RestResource (
 *   id = "custom_module_create_employee",
 *   label = @Translation("Create Employee"),
 *   uri_paths = {
 *     "canonical" = "/api/custom-module-create-employee/{id}",
 *     "create" = "/api/custom-module-create-employee"
 *   }
 * )
 *
 * @DCG
 ...
 */
class CreateEmployeeResource extends ResourceBase implements DependentPluginInterface {
  ...
  public function post($data) {
    ...
    //$this->validate($data);
    $id = $this->dbConnection->insert('employee_data')
      ->fields($data)
      ->execute();
    $this->logger->notice('New create employee record has been created.');
  ...
  public function delete($id) {
    ...
    $this->dbConnection->delete('employee_data')
      ->condition('id', $id)
      ->execute();

    $this->logger->notice('Create Employee record @id has been deleted.', ['@id' => $id]);
  ...
  protected function loadRecord($id) {
    $record = $this->dbConnection->query('SELECT * FROM {employee_data} WHERE id = :id', [':id' => $id])->fetchAssoc();
    if (!$record) {
  ...
  protected function updateRecord($id, array $record) {
    ...
    $this->dbConnection->update('employee_data')
      ->fields($record)
      ->condition('id', $id)
      ->execute();

    $this->logger->notice('Create Employee record @id has been updated.', ['@id' => $id]);
