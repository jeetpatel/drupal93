...
/**
 * Represents List Learning records as resources.
 *
 * @RestResource (
 *   id = "learning_list_learning",
 *   label = @Translation("List Learning"),
 *   uri_paths = {
 *     "canonical" = "/api/learning-list-learning/{id}",
 *     "https://www.drupal.org/link-relations/create" = "/api/learning-list-learning"
 *   }
 * )
 *
 * @DCG
 ...
 */
class ListLearningResource extends ResourceBase implements DependentPluginInterface {
  ...
  public function post($data) {
    ...
    $id = $this->dbConnection->insert('learning_list_learning')
      ->fields($data)
      ->execute();

    $this->logger->notice('New list learning record has been created.');
  ...
  public function delete($id) {
    ...
    $this->dbConnection->delete('learning_list_learning')
      ->condition('id', $id)
      ->execute();

    $this->logger->notice('List Learning record @id has been deleted.', ['@id' => $id]);
  ...
  protected function loadRecord($id) {
    $record = $this->dbConnection->query('SELECT * FROM {learning_list_learning} WHERE id = :id', [':id' => $id])->fetchAssoc();
    if (!$record) {
  ...
  protected function updateRecord($id, array $record) {
    ...
    $this->dbConnection->update('learning_list_learning')
      ->fields($record)
      ->condition('id', $id)
      ->execute();

    $this->logger->notice('List Learning record @id has been updated.', ['@id' => $id]);
