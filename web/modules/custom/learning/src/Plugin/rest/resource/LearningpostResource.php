...
/**
 * Represents LearningPost records as resources.
 *
 * @RestResource (
 *   id = "learning_learningpost",
 *   label = @Translation("Learning GET/Post"),
 *   uri_paths = {
 *     "canonical" = "/api/learning-learningpost/{id}",
 *     "https://www.drupal.org/link-relations/create" = "/api/learning-learningpost"
 *   }
 * )
 *
 * @DCG
 ...
 */
class LearningpostResource extends ResourceBase implements DependentPluginInterface {
  ...
  public function post($data) {
    ...
    $id = $this->dbConnection->insert('learning_learningpost')
      ->fields($data)
      ->execute();

    $this->logger->notice('New learningpost record has been created.');
  ...
  public function delete($id) {
    ...
    $this->dbConnection->delete('learning_learningpost')
      ->condition('id', $id)
      ->execute();

    $this->logger->notice('LearningPost record @id has been deleted.', ['@id' => $id]);
  ...
  protected function loadRecord($id) {
    $record = $this->dbConnection->query('SELECT * FROM {learning_entity} WHERE id = :id', [':id' => $id])->fetchAssoc();
    if (!$record) {
  ...
  protected function updateRecord($id, array $record) {
    ...
    $this->dbConnection->update('learning_learningpost')
      ->fields($record)
      ->condition('id', $id)
      ->execute();

    $this->logger->notice('LearningPost record @id has been updated.', ['@id' => $id]);
