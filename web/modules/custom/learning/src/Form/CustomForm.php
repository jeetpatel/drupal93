...
class CustomForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $nojs = NULL) {
    //$form = \Drupal::formBuilder()->getForm(ConditionalForm::class);
    ...
    if ($nojs == NULL) {
      ...
      $form['country_dialog_callback'] = [
        '#type' => 'container',
        '#attributes' => ['id' => 'country_dialog_callback'],
      ];
      ...
      $form['ajax_link'] = [
        ...
        '#url' => Url::fromUserInput('/learning/param/1'),
      ];
      $form['model_dialog'] = [
        '#type' => 'link',
        ...
        '#attributes' => [
          ...
          'id' => 'ajax-example-modal-link',
          'data-ajax-wrapper' => 'debug_container_wrapper'
        ],
        '#url' => Url::fromRoute('<front>', ['nojs' => 'nojs']),
      ];
  ...
  public function myAjaxCallback(array &$form, FormStateInterface $form_state) {
    ...
    if ($selectedValue = $form_state->getValue('state')) {
      ...
      $form['state_list']['#options'] = ['11' => '11', '22' => '22'];
    }
    //sleep(2);
  ...
  public function myAjaxDialogCallback(array &$form, FormStateInterface $form_state) {
    ...
    $elem = [
      ...
      '#size' => '60',
      '#disabled' => TRUE,
      '#value' => "I am a new textfield: $selectedValue!",
      '#attributes' => [
        'id' => ['country_dialog_callback'],
      ],
    ];
    ...
    //$response->addCommand(new ReplaceCommand('#country_dialog_callback', $renderedField));
    // Custom javascript code.
    $response->addCommand(new InvokeCommand(NULL, 'myAjaxCallback', ['Country::' . $selectedValue]));
    // Show the dialog box.
    //$response->addCommand(new OpenModalDialogCommand('My title', $dialogText, ['width' => '300']));
    $debugOut = @\Kint::dump($elem);
    $response->addCommand(new ReplaceCommand('#debug_container_wrapper', '<div id="debug_container_wrapper1">' . $debugOut . '</div>'));
  ...
  public function myExampleAjaxCallback(array &$form, FormStateInterface $form_state) {
    ...
    //$build['#markup'] = '<div id="edit-output"><h1>' . $selectedText . '</h1></div>';
    //return $build;
    $response = new AjaxResponse();
    ...
    // $content = '<p>Insert Command content</p>';
    // $settings = ['my-setting' => 'setting'];
    // $response->addCommand(new InsertCommand($selector, $content, $settings));

    $dialogText['#attached']['library'][] = 'core/drupal.dialog.ajax';
    $dialogText['#markup'] = "Selected value:: $selectedText";
    $title = 'Open Modal Dialog Command';
    ...
    $response->addCommand(new OpenModalDialogCommand($title, $dialogText, ['width' => '300']));
